...
class SpondMCPServer:
    ...
    async def initialize_api(
        self, bearer_token: str = None, club_id: str = None
    ) -> bool:
    ...
    async def get_member_payment_summary(self, member_name: str) -> Dict[str, Any]:
        ...
        if not self.api:
            return {
                "error": "API not initialized. Please provide bearer_token and club_id."
            }
        ...
        try:
            ...
            granular_rows, stats = self.report_generator.process_payment_data(
                payments, member_map, self.api
            )

            # Filter for this specific member
            member_payments = [
                row for row in granular_rows if row.get("Member ID") == member_id
            ]
            ...
            # Calculate totals
            total_owed = sum(
                float(payment.get("Amount", 0)) for payment in member_payments
            )
            ...
            for payment in member_payments:
                ...
                payment_types[payment_type]["total_amount"] += float(
    ...
    async def get_all_outstanding_payments(
        self, title_filter: str = None, limit: int = 50
    ) -> Dict[str, Any]:
    ...
    async def get_payment_statistics(self, title_filter: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            for row in filtered_rows:
                ...
                if payment_type not in payment_types:
                    payment_types[payment_type] = {"count": 0, "total_amount": 0}
                payment_types[payment_type]["count"] += 1
                payment_types[payment_type]["total_amount"] += float(
                    row.get("Amount", 0)
                )
        ...
        try:
            ...
            for member_id, name in member_map.items():
                if query_lower in name.lower():
                    matching_members.append({"id": member_id, "name": name})
    ...
    async def handle_tool_call(
        ...
        if tool_name == "get_member_payment_summary":
            ...
            return await self.get_all_outstanding_payments(
                parameters.get("title_filter"), parameters.get("limit", 50)
            )
